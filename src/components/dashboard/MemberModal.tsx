...
const OUTCOME_STYLE: Record<Outcome, string> = {
  ...
  provisional_success: 'bg-emerald-100 text-emerald-800 ring-emerald-200',
  fail: 'bg-rose-100 text-rose-800 ring-rose-200',
  provisional_fail: 'bg-white/70 text-slate-600 ring-black/5',
          ...
          <div className="relative overflow-hidden bg-[#f6f1e8] px-5 pb-6 pt-5 text-slate-900">
            ...
            <div className="relative flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-xl font-extrabold leading-tight tracking-tight">
                  {m.nickname}
                  {isMe && <span className="ml-1.5 align-middle rounded-full bg-red-600 px-1.5 py-0.5 text-[10px] font-semibold text-white">나</span>}
                </p>
            ...
            <div className="relative mt-5">
              ...
              <div className="mt-2 flex h-2.5 w-full overflow-hidden rounded-full bg-slate-900/10" role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100} aria-label={`${m.nickname} 진행률`}>
                <div className="h-full rounded-full bg-gradient-to-r from-red-600 to-orange-400 transition-all" style={{ width: `${pct}%` }} />
                {pendingPct > 0 && <div className="h-full bg-amber-300" style={{ width: `${pendingPct}%` }} title="승인 대기" />}
              </div>
                ...
                {m.days.map((d, i) => {
                  ...
                  return (
                    <div key={d.date} className="flex h-full flex-1 flex-col items-center gap-1" title={`${d.date} · ${formatMeters(total)} km`}>
                      ...
                      <span className={`text-[10px] ${isToday ? 'rounded-full bg-slate-900 px-1.5 font-semibold text-white' : i >= 5 ? 'text-red-400' : 'text-slate-500'}`}>{DOW[i]}</span>
                    </div>
                  );
                })}
              </div>
